...
void draw_vertical_gradient(UIColor *topColor, UIColor *bottomColor, CGRect rect)
{
...
NSArray *filter_filenames(NSArray *unfilteredFiles, NSArray *types)
{
	...
	while ((filename = [fileEnumerator nextObject]))
	{
		...
		hfsFileType = [[dic valueForKey:NSFileHFSTypeCode] stringByTrimmingCharactersInSet:
		...
		while(temp_addr != NULL)  { 
			if(temp_addr->ifa_addr->sa_family == AF_INET)  { 
				...
				if([[NSString stringWithUTF8String:temp_addr->ifa_name] isEqualToString:@"en0"])  { 
					...
					host = [NSString stringWithUTF8String:inet_ntoa(((struct sockaddr_in *)temp_addr->ifa_addr)->sin_addr)]; 
				} 
			} 
